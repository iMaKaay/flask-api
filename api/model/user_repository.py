...
class UserRepository(Repository):
    ...
    def is_invalid(self, user: User) -> list:
        ...
        if not user.name:
            invalid.append({"name": "must be filled"})
        ...
        # verify if there is another user with the same email
        user_checking_email = self.get_by_email(user.email)
        if user_checking_email:
            if (not user.id) or (user.id != user_checking_email.id):
