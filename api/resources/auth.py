...
from api.exceptions import TokenNotFound
from api.model import User, Token
from api.model import UserRepository, TokenRepository
...
def register() -> Response:
    ...
    if not is_invalid:
        ...
        response = makeResp.success(user.serialize(), 'User created successfully')
    else:
        response = makeResp.error(is_invalid)
...
def login() -> Response:
    ...
    if not user:
        response = makeResp.error('Username or Password not valid', False, 401)

    else:
        access_token_encoded = create_access_token(identity=user.username, expires_delta=timedelta(days=1))
